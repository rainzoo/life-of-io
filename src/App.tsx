...
import {
	...
	type LayerId,
	LayerLane,
} from "@/components/viz/LayerLane";
import { PhaseBadge, type PhaseId } from "@/components/viz/PhaseBadge";
...
const STEPS: VisualizationStep[] =
	visualizationData.steps as VisualizationStep[];
				...
				<header className="flex items-center justify-center gap-4 border-b border-border/60 px-4 py-3 md:px-6 relative">
					...
					<div className="text-center">
						<h1 className="text-2xl md:text-3xl font-bold tracking-tight mb-2">
							<span className="gradient-text">Life of IO</span>
						</h1>
									...
									<div key={group.id} className="space-y-2">
										...
										<div className="space-y-1.5 pl-2 border-l-2 border-slate-700/50 relative">
											{groupLayers.map((layer, index) => {
												...
												const nextIsActive =
													nextLayer && activeLayerIds.has(nextLayer.id);
										...
										<div className="flex items-center gap-2">
											{currentStep.keyConcept && (
												<motion.span
													...
													className="rounded-full bg-sky-500/20 px-2 py-0.5 text-[0.65rem] font-medium text-sky-300"
												>
													{currentStep.keyConcept}
												</motion.span>
											)}
							...
							<CardContent className="space-y-4">
								...
								<div>
									<p className="max-w-2xl text-[0.75rem] text-muted-foreground md:text-sm mx-auto">
										Use the controls to walk step by step through the life of a
										single write.
									</p>
								</div>
							</CardContent>
											...
											return (
												...
												>
													<div className="flex items-center gap-3">
														<span
															className={`font-mono text-sm font-medium ${phaseColor}`}
														>
															{step.stepLabel}
														</span>
